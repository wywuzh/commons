 ...
 * @since JDK 1.7
 */
public class AsyncCurdRepository<E extends AbstractEntity, V extends AbstractVo>
        extends AsyncPaginationRepository<E, V> {
    ...
    public List<E> queryEntityList(V vo) throws SQLException {
        ...
        return this.queryForList(getTableName(), getQueryColumns(), conditionColumns.toArray(new String[0]),
                conditionObjs.toArray());
    }
    ...
    public long queryEntityTotal(V vo) throws SQLException {
        ...
        return this.queryForLong(getTableName(), "COUNT(1)", conditionColumns.toArray(new String[0]),
                conditionObjs.toArray());
    }
    ...
    public PaginationObject<E, V> queryForPager(PaginationParameter<V> paginationParamter) throws SQLException {
        ...
        // 查询数据总数
        Long rowCount = this.queryForLong(getTableName(), "COUNT(1)", conditionColumns.toArray(new String[0]),
                conditionObjs.toArray());
        List<E> resultList = new ArrayList<E>();
        if (rowCount > 0) {
            // 查询数据
            resultList = this.queryForList(getTableName(), getQueryColumns(), conditionColumns.toArray(new String[0]),
                    conditionObjs.toArray(), paginationParamter.getPageNo(), paginationParamter.getPageSize());
        }
