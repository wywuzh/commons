 ...
 * @since JDK 1.7
 */
public class AsyncPaginationRepository<E extends AbstractEntity, V extends AbstractVo>
        extends AsyncBasicRepository<E, V> {
     ...
     */
    public List<E> queryForList(String tableName, String columnSql, String conditionSql, Object[] conditionObjs,
                                Sort[] sorts, int pageNo, int pageSize) throws SQLException {
        Assert.notNull(tableName, "tableName must not be null");
        Assert.notNull(columnSql, "columnSql must not be null");
        ...
        try {
            return getAsyncQueryRunner().query(connection,
                    generatePageSql(querySql.toString(), paginationParameter),
                    new BeanListHandler<E>(getEntityClass()), conditionObjs).get();
