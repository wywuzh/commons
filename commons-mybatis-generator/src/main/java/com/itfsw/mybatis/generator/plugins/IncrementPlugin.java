...
public class IncrementPlugin extends BasePlugin implements IIncrementPluginHook {
  ...
  public XmlElement generateIncrementSet(IntrospectedColumn introspectedColumn, String prefix, boolean hasComma) {
    if (this.supportIncrement(introspectedColumn)) {
      ...
      TextElement spec = new TextElement(
          MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " = " + MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " ${" + columnMap + "."
              + FIELD_OPERATE_FOR_CLASS_INCREMENT + "} " + XmlElementGeneratorTools.getParameterClause(columnMap + "." + FIELD_VALUE_FOR_CLASS_INCREMENT, introspectedColumn) + (hasComma ? "," : ""));
      ...
      otherwiseEle.addElement(new TextElement(
          MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " = " + MyBatis3FormattingUtilities.getParameterClause(introspectedColumn, prefix) + (hasComma ? "," : "")));
  ...
  public XmlElement generateIncrementSetSelective(IntrospectedColumn introspectedColumn, String prefix) {
    if (this.supportIncrement(introspectedColumn)) {
      ...
      XmlElement whenIncEle = new XmlElement("when");
      whenIncEle.addAttribute(new Attribute("test", columnMap + " != null"));
      TextElement spec = new TextElement(MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " = " + MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " ${"
          + columnMap + "." + FIELD_OPERATE_FOR_CLASS_INCREMENT + "} " + XmlElementGeneratorTools.getParameterClause(columnMap + "." + FIELD_VALUE_FOR_CLASS_INCREMENT, introspectedColumn) + ",");
      whenIncEle.addElement(spec);
      choose.addElement(whenIncEle);
      ...
      whenEle
          .addElement(new TextElement(MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " = " + MyBatis3FormattingUtilities.getParameterClause(introspectedColumn, prefix) + ","));
      ...
      for (IntrospectedColumn incColumn : this.incColumns) {
        ...
        for (IntrospectedColumn column : columns) {
          if (incColumn.getActualColumnName().equals(column.getActualColumnName())) {
            ...
            when.addElement(new TextElement("${column.escapedColumnName} = ${column.escapedColumnName} " + "${" + columnMap + "." + FIELD_OPERATE_FOR_CLASS_INCREMENT + "} "
  ...
  private InnerEnum generateIncrementEnum(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
    ...
    // 添加增减方法
    Method mInc = JavaElementGeneratorTools.generateMethod(METHOD_INC, JavaVisibility.PUBLIC, new FullyQualifiedJavaType(ENUM_INCREMENT + "." + CLASS_INCREMENT_ITEM),
    ...
    Method mDec = JavaElementGeneratorTools.generateMethod(METHOD_DEC, JavaVisibility.PUBLIC, new FullyQualifiedJavaType(ENUM_INCREMENT + "." + CLASS_INCREMENT_ITEM),
        new Parameter(FullyQualifiedJavaType.getObjectInstance(), "value"));
  ...
  private void addIncMethodToTopLevelClass(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
    // 增加field
    Field fIncrements = JavaElementGeneratorTools.generateField(IncrementPlugin.FIELD_INC_MAP, JavaVisibility.PROTECTED, new FullyQualifiedJavaType("Map<String, Object>"), "new HashMap<>()");
    ...
    // 增加自增方法
    Method mIncrement = JavaElementGeneratorTools.generateMethod(METHOD_INCREMENT, JavaVisibility.PUBLIC, topLevelClass.getType(),
  ...
  private InnerClass generateIncrementItemClass(IntrospectedTable introspectedTable) {
    ...
    // 添加构造函数
    Method mConstructor = JavaElementGeneratorTools.generateMethod(CLASS_INCREMENT_ITEM, JavaVisibility.PUBLIC, incCls.getType(),
        new Parameter(columnField.getType(), FIELD_COLUMN_FOR_CLASS_INCREMENT), new Parameter(operateField.getType(), FIELD_OPERATE_FOR_CLASS_INCREMENT),
