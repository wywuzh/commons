...
public class IncrementsPlugin extends BasePlugin implements IModelBuilderPluginHook, IIncrementsPluginHook, ILombokPluginHook {
    ...
    private boolean lombokBuilderClassGenerated(TopLevelClass topLevelClass, List<IntrospectedColumn> columns, IntrospectedTable introspectedTable) {
        if (this.support()) {
            ...
            if (find) {
                ...
                Method builderMethod = JavaElementGeneratorTools.generateMethod("builder", JavaVisibility.PUBLIC, builderType);
                ...
                builderCls.getType().addTypeArgument(new SpecTypeArgumentsFullyQualifiedJavaType("<C extends " + topLevelClass.getType().getShortName() + ", B extends "
                        + topLevelClass.getType().getShortName() + "." + topLevelClass.getType().getShortName() + "Builder<C, B>>"));
                ...
                // self 方法
                Method selfMethod1 = JavaElementGeneratorTools.generateMethod("self", JavaVisibility.PROTECTED, new FullyQualifiedJavaType("B"));
                ...
                // build 方法
                Method buildMethod1 = JavaElementGeneratorTools.generateMethod("build", JavaVisibility.PUBLIC, new FullyQualifiedJavaType("C"));
                ...
                if (this.incEnum == null) {
                    ...
                    // Builder 中 添加字段支持
                    Field fIncrements = JavaElementGeneratorTools.generateField(IncrementsPlugin.FIELD_INC_MAP, JavaVisibility.PROTECTED,
                ...
                for (IntrospectedColumn column : columns) {
                    if (this.supportIncrement(column)) {
                        Field field = JavaBeansUtil.getJavaBeansField(column, context, introspectedTable);
                        // 增加方法
                        Method mIncrements = JavaElementGeneratorTools.generateMethod(field.getName(), JavaVisibility.PUBLIC, builderType2, new Parameter(field.getType(), field.getName()),
                ...
                // self 方法
                Method selfMethod = JavaElementGeneratorTools.generateMethod("self", JavaVisibility.PROTECTED,
                ...
                // build 方法
                Method buildMethod = JavaElementGeneratorTools.generateMethod("build", JavaVisibility.PUBLIC, topLevelClass.getType());
        ...
        if (this.support()) {
            ...
            for (IntrospectedColumn column : columns) {
                if (this.supportIncrement(column)) {
                    Field field = JavaBeansUtil.getJavaBeansField(column, context, introspectedTable);
                    // 增加方法
                    Method mIncrements = JavaElementGeneratorTools.generateMethod(field.getName(), JavaVisibility.PUBLIC, builderClass.getType(), new Parameter(field.getType(), field.getName()),
    ...
    public List<Element> incrementSetElementGenerated(IntrospectedColumn introspectedColumn, String prefix, boolean hasComma) {
        ...
        if (this.supportIncrement(introspectedColumn)) {
            ...
            when.addAttribute(new Attribute("test", (prefix != null ? prefix : "_parameter.") + IncrementsPlugin.METHOD_INC_CHECK + "('"
                    + MyBatis3FormattingUtilities.escapeStringForMyBatis3(introspectedColumn.getActualColumnName()) + "')"));
            TextElement spec = new TextElement(MyBatis3FormattingUtilities.getEscapedColumnName(introspectedColumn) + " ${" + (prefix != null ? prefix : "_parameter.")
            ...
            for (IntrospectedColumn incColumn : this.incColumns) {
                ...
                for (IntrospectedColumn column : columns) {
                    if (incColumn.getActualColumnName().equals(column.getActualColumnName())) {
                        ...
                        when.addElement(new TextElement("${column.escapedColumnName} = ${column.escapedColumnName} ${record." + METHOD_GET_INC_MAP + "()." + incColumn.getActualColumnName()
                                + ".value} " + XmlElementGeneratorTools.getParameterClause("record.${column.javaProperty}", incColumn)));
                        results.add(when);
                    }
                }
            }
    ...
    private void addIncMethodToTopLevelClass(TopLevelClass topLevelClass, InnerClass builderCls, IntrospectedTable introspectedTable, boolean withLombok) {
        ...
        if (withLombok) {
            ...
            Method getIncMapMethod = JavaElementGeneratorTools.generateMethod(METHOD_GET_INC_MAP, JavaVisibility.PUBLIC, fIncrements.getType());
        ...
        // 增加判断方法
        Method mHasIncsForColumn = JavaElementGeneratorTools.generateMethod(IncrementsPlugin.METHOD_INC_CHECK, JavaVisibility.PUBLIC, FullyQualifiedJavaType.getBooleanPrimitiveInstance(),
            ...
            for (Element ele : xmlElement.getElements()) {
                ...
                if (ele instanceof TextElement) {
                    ...
                    if (text.matches("(^set\\s)?\\S+\\s?=.*")) {
                        ...
                        IntrospectedColumn introspectedColumn = IntrospectedTableTools.safeGetColumn(introspectedTable, columnName);
                        // 查找判断是否需要进行节点替换
                        List<Element> incrementEles = PluginTools.getHook(IIncrementsPluginHook.class).incrementSetElementGenerated(introspectedColumn, hasPrefix ? "record." : null,
                                text.endsWith(","));
                        if (!incrementEles.isEmpty()) {
