...
public class AppendFileUtils {
     ...
     * @param content  内容
     */
    public static void appendContentForBw(File destFile, String content) throws IOException {
        Assert.notNull(destFile, "参数destFile不能为空");
        Assert.notNull(content, "参数content不能为空");

        try (BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(destFile, true), StandardCharsets.UTF_8))) {
            out.write(content);
            // 确保内容写入磁盘（通常close()会自动flush）
            out.flush();
        } catch (IOException e) {
            log.error("destFile={} 使用BufferedWriter方式追加文件失败", destFile, e);
            throw e;
        }
    }
     ...
     * @param destFile 目标文件
     * @param content  内容
     * @see #appendContentForBw(File, String) 使用BufferedWriter方式追加文件内容
     */
    public static void appendContentForFw(File destFile, String content) throws IOException {
        Assert.notNull(destFile, "参数destFile不能为空");
        Assert.notNull(content, "参数content不能为空");
        ...
        try (FileWriter writer = new FileWriter(destFile, true)) {
            ...
            log.error("destFile={} 使用FileWriter方式追加文件失败", destFile, e);
            throw e;
        }
    }
     ...
     * @param content  追加的内容
     */
    public static void appendContentForRaf(File destFile, String content) throws IOException {
        Assert.notNull(destFile, "参数destFile不能为空");
        Assert.notNull(content, "参数content不能为空");

        // 打开一个随机访问文件流，按读写方式
        try (RandomAccessFile randomFile = new RandomAccessFile(destFile, "rw")) {
            // 将写文件指针移到文件尾。文件长度，字节数
            randomFile.seek(randomFile.length());
            // 使用明确编码写入内容，避免乱码问题
            randomFile.write(content.getBytes(StandardCharsets.UTF_8));
        } catch (IOException e) {
            log.error("destFile={} 使用RandomAccessFile方式追加文件失败：", destFile, e);
            throw e;
        }
    }
