...
public class CalcExpressionUtils {
  ...
  public static BigDecimal getCalcValue(final String calcFieldName, final String calcFieldTitle, final String calcExpression, Object target) {
    ...
    resolvedCalcExpression = resolvedCalcExpression.replaceAll("and+", "&&").replaceAll("or+", "||").replaceAll("<>+", "!=").replaceAll("\\s", "");
    // 中英文冒号去掉
    resolvedCalcExpression = resolvedCalcExpression.replaceAll(":+", "").replaceAll("：+", "");
    // 中文括号去掉
    resolvedCalcExpression = resolvedCalcExpression.replaceAll("（+", "").replaceAll("）+", "");
    ...
    try {
      ...
      LOGGER.error("calcFieldName={}, calcFieldTitle={}, calcExpression={}, resolvedCalcExpression={}, target={} 公式解析失败，结果值={}", calcFieldName, calcFieldTitle, calcExpression, resolvedCalcExpression,
          JsonMapper.DEFAULT_JSON_MAPPER.toJson(target), value);
      throw new ExpressionException("【" + calcExpression + "】 公式解析失败！");
    }
  }
